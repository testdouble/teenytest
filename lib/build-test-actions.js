...
var actionsForExampleGroup = function (exampleGroup, ancestors, helper, stats) {
  return [
    actionForHook(exampleGroup.beforeAll, (exampleGroup.name || 'module') + ' beforeAll', null, exampleGroup.file),
    actionsForExampleGroupItems(exampleGroup, ancestors, helper, stats),
    actionForHook(exampleGroup.afterAll, (exampleGroup.name || 'module') + ' afterAll', null, exampleGroup.file)
  ]
}

var actionsForExampleGroupItems = function (exampleGroup, ancestors, helper, stats) {
  return _.map(exampleGroup.items, function (item) {
...
var actionsForTestFunctions = function (test, ancestors, helper, stats) {
  return [
    ...
    _.map(ancestors, function (ancestor) {
      return actionForHook(ancestor.beforeEach, (ancestor.name || 'module') + ' beforeEach', test.context, ancestor.file)
    }),
    ...
    _(ancestors).reverse().map(function (ancestor) {
      return actionForHook(ancestor.afterEach, (ancestor.name || 'module') + ' afterEach', test.context, ancestor.file)
    }).value()
  ]
}
