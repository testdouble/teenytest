...
var actionsForExampleGroup = function (exampleGroup, ancestors, helper) {
  return [
    actionForHook(exampleGroup.beforeAll, exampleGroup.name + ' beforeAll', null, exampleGroup.file),
    actionsForExampleGroupItems(exampleGroup, ancestors, helper),
    actionForHook(exampleGroup.afterAll, exampleGroup.name + ' afterAll', null, exampleGroup.file)
  ]
}

var actionsForExampleGroupItems = function (exampleGroup, ancestors, helper) {
  return _.map(exampleGroup.tests, function (item) {
    if (item.type === 'exampleGroup') {
      ...
      return actionsForTestFunctions(item, ancestors.concat(exampleGroup), helper)
    }
  })
}

var actionsForTestFunctions = function (test, ancestors, helper) {
  return [
