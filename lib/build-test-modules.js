...
module.exports = function (testGlob, cwd) {
  ...
  if (testGlob.indexOf('#') > -1) {
    var testSelection = testGlob.split('#')
    testGlob = testSelection[0]
    testName = testSelection[1]
  }

  return _.map(glob.sync(testGlob), function (file) {
    var testModule = require(path.resolve(cwd, file))
    var tests = _.map(selectedTestFunctionsIn(testModule, testName), function (testEntry, i) {
      currentTestOrdinal++
...
var selectedTestFunctionsIn = function (testModule, selectedTestName) {
  ...
  return filterTestFunctionsByName(testFunctions, selectedTestName)
}

var filterTestFunctionsByName = function (testFunctions, selectedTestName) {
  return _.filter(testFunctions, function (testFunction) {
    return testFunction.testName === selectedTestName
  })
}
