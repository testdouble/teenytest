...
module.exports = {
  ...
  getResultFor: function (metadata) {
    return results.all[idFor(metadata)]
  },
  isPassing: function () {
  ...
  deepestErrorsAssociatedWithATest: function () {
    return _(_(results.currentNesting).map('errors').last())
  ...
  nest: function (metadata) {
    var newlyNestedResult = results.all[idFor(metadata)] = _.assign({},
